...
const ApprovalPending = () => {
    const { PendingApprovalComp, checkStatus, ApproveComplaint, DenyComplaint, ResolveComplaint } = useContext(ComplaintContext);
    ...
    useEffect(() => {
        const calc = async () => {
            const data = await PendingApprovalComp();
                ...
                <div className="max-w-sm mx-auto py-3 space-y-3 text-center">
                    ...
                    {!statusData ? (<div className="text-gray-800 mt-[5%]">Sorry , We cant find any Complaint with the given Id , Pls check the Id entered</div>) : (
                        <div className="px-4">
                            ...
                            <input type="text" onChange={(e) => setremark(e.target.value)} placeholder="Remark" className="w-full pl-5 pr-3 py-2 text-black bg-slate-100 outline-none border focus:border-indigo-600 shadow-sm rounded-lg" />
                            </div>
